...
## Introduction
In this first lab, you will learn the basics of R. We will start with how to create a script, then we will discuss 
...
Note: You will be submitting this script on Gauchospace to be graded. The only parts that will count towards your grade are the answers to the questions (marked in red). You may include the rest of in your submitted script, but it is not required for points. If you prefer to keep your question responses in a separate script without the in-between code for clarity, this is fine. If you do not separate the please **demarcate your responses to the questions clearly**, using a format like the example script included. The example script is available on rstudio.cloud as well as on Gauchospace.
...
Create a new script. (GET INSTRUCTIONS)
Name the script when you save it with your last name and first initial. This script is where you will be writing your code. Your answers to the questions that count for points (marked in red) will also go into the script. As noted above, the script you submit to Gauchospace, the only code and other writing that will count for points will be the responses to the questions marked in red. So you can choose whether to keep those responses in a separate script or whether you want to keep it all together. 
...
##### <span style="color:red">Q1. In your script, write out at least four calculations besides the examples I gave. Run each and paste the answers into a comment to the right of each equation. (4) </span>
...
b <- 2
```
Once you run these two lines, R will know that if you enter `a` you are referring to the number `1`, and if you type `y` you are referring to the number `2`. 


**NOTE:** Now look at your `Environment` pane. Do you see your two newly-created variables `a` and `y`? Do you see how `a` has taken on the value `1` and `y` has taken on the value `2`?  
...
After running the variable assignment lines, any time you enter `a` or `b` it will  treat them **exactly the same** as if you typed the number `1` or `2`.
...
##### <span style="color:red">Q2. In your script, answer the following questions. Include your code and the answers in a comment to the right of the equation. (A) What is b divided by a? (B) What is b-cubed? (2) </span>
...
```
The characters between a set of quotes are called a "string". DON'T FORGET THE QUOTATION MARKS ARE VERY IMPORTANT IF YOU ARE USING STRINGS. 
...
##### <span style="color:red">Q3. Show me the **code** to return the values of `range15` that are greater than 12. (2) </span>
...
!is.numeric(mynumeric)
```
The `is.numeric()` function tests whether or not the value stored in `a` is numeric. If it is, it returns `TRUE`, if it is *not*, it will return `FALSE`. By putting `!` in front of it, we "flipped" the question. `!is.numeric()` is asking if `a` is NOT numeric. Putting `!` before any logical test changes it to asking for the "flipped" version of whatever the question is. 
...
```
The functions `is.character()` tests whether or not the value stored is character type. When you run this line, you should get `TRUE` in your console. 
...
##### <span style="color:red">Q4. Look up `Logic` in the help system (it is case-sensitive), then click "Logical Operators". What does an exclamation point indicate? Answer in a comment. (1) </span>
